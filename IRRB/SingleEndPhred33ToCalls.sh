...
do
	...
	qsub -N BaseRecalibration_${parentDir} -hold_jid ${DEPENDENCYSTRING#,} ${GLOBALQSUBOPTIONS} -l num_threads=${GATKNUMTHREADS} -l mem_free=${JAVAMEMORY} -o ${LOGPATH}BaseRecalibration_${parentDir}.o -e ${LOGPATH}BaseRecalibration_${parentDir}.e ${RIGPATH}jobScripts/BaseRecalibrationMultiSampleLanejob.sh ${JAVAMEMORY} ${GATKPATH} ${GATKNUMTHREADS} ${REFERENCEFASTA} ${INTERVALFILE} ${WGSREFERENCE} ${OUTPUTPATH}${parentDir}-realigned.recal.table ${LANEFILEARRAY[@]} >& ${LOGPATH}qsub.tmp
	...
	do
		...
		qsub -N ApplyRecalibration_${sampleID} -hold_jid BaseRecalibration_${parentDir} ${GLOBALQSUBOPTIONS} -l num_threads=1 -l mem_free=${JAVAMEMORY} -o ${LOGPATH}ApplyRecalibration_${sampleID}.o -e ${LOGPATH}ApplyRecalibration_${sampleID}.e ${RIGPATH}jobScripts/ApplyRecalibrationjob.sh ${JAVAMEMORY} ${GATKPATH} ${GATKNUMTHREADS} ${REFERENCEFASTA} ${INTERVALFILE} ${OUTPUTPATH}${parentDir}-realigned.recal.table ${file} ${OUTPUTPATH}${sampleID}.recalibrated.bam >& ${LOGPATH}qsub.tmp
        ...
        qsub -N BaseRecalibration2_${parentDir} -hold_jid ${DEPENDENCYSTRING#,} ${GLOBALQSUBOPTIONS} -l num_threads=${GATKNUMTHREADS} -l mem_free=${JAVAMEMORY} -o ${LOGPATH}BaseRecalibration2_${parentDir}.o -e ${LOGPATH}BaseRecalibration2_${parentDir}.e ${RIGPATH}jobScripts/BaseRecalibrationMultiSampleLanejob.sh ${JAVAMEMORY} ${GATKPATH} ${GATKNUMTHREADS} ${REFERENCEFASTA} ${INTERVALFILE} ${WGSREFERENCE} ${OUTPUTPATH}${parentDir}-recalibrated.recal.table ${RECALIBRATEDARRAY[@]} >& ${LOGPATH}qsub.tmp
	...
	do
		...
		qsub -N HaploCall_${sampleID} -hold_jid BaseRecalibration2_${parentDir} ${GLOBALQSUBOPTIONS} -l num_threads=1 -l mem_free=${JAVAMEMORY} -o ${LOGPATH}HaploCall_${sampleID}.o -e ${LOGPATH}HaploCall_${sampleID}.e ${RIGPATH}jobScripts/HaploCallerjob.sh ${JAVAMEMORY} ${GATKPATH} 1 ${REFERENCEFASTA} ${INTERVALFILE} ${OUTPUTPATH}${sampleID}.recalibrated.bam ${OUTPUTPATH}${sampleID}.GVCF.vcf >& ${LOGPATH}qsub.tmp 
        ...
        qsub -N Cleanup_${sampleID} -hold_jid ${DEPENDENCYSTRING#,} ${GLOBALQSUBOPTIONS} -l num_threads=1 -l mem_free=1g -o ${LOGPATH}Cleanup_${sampleID}.o -e ${LOGPATH}Cleanup_${sampleID}.e ${RIGPATH}jobScripts/CleanupIntermediatesjob.sh ${REMOVALARRAY[@]} >& ${LOGPATH}qsub.tmp
